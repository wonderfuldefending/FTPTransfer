    ...
    public class clsFTP
    {
        ...
        private FileStruct ParseFileStructFromWindowsStyleRecord(string Record)
        {
            ...
            else
            {
                ...
                if (strs.Length <= 2) // 文件名包含空格 eg. 3451320 EXCEL_HPE.xlsx
                    processstr = strs[1];
                else // 文件名包含空格 eg : 3451320 EXCEL_HPE WH_17-18 May.xlsx
                {
        ...
        public byte[] GetUploadZipStream(string RemoteFile)
        {
            byte[] outPut = null;
            byte[] bt = DownloadFile(RemoteFile);
            ...
            {
                ZipEntry ent = new ZipEntry(RemoteFile);
        ...
        public bool ZipFileToAnotherDirectory(string RemoteFile, string DirectoryName)
        {
            string CurrentWorkDir = this.DirectoryPath;
            byte[] outPut = null;
            ...
            try
            {
                ...
                GotoDirectory(DirectoryName);
                bool Success = UploadFile(outPut, Path.GetFileNameWithoutExtension(RemoteFile) + ".zip", true);
                this.DirectoryPath = CurrentWorkDir;
                return true;
